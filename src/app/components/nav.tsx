      ...
      <div className="px-5 sm:px-8 lg:px-14 h-16 flex items-center justify-between gap-4">
                ...
                <a
                  ...
                  className={`relative text-[13px] tracking-wide transition-colors ${
                    isActive
                      ...
                      : "text-[#5F6A64] dark:text-[#B8BFB9] hover:text-[#A07F2E] dark:hover:text-[#C9A24B]"
                  }`}
                >
                  {l.label}
                  <span
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <button
            ...
            className="w-10 h-10 md:w-9 md:h-9 rounded-full border border-[#2A3530]/15 dark:border-[#E8E6DF]/15 flex items-center justify-center text-[#5F6A64] dark:text-[#E8E6DF] hover:border-[#C9A24B] hover:text-[#A07F2E] dark:hover:text-[#C9A24B] transition-colors cursor-pointer"
          >
        ...
        <div className="px-5 pt-2 pb-6 border-t border-[#2A3530]/15 dark:border-[#E8E6DF]/12 bg-[#EFEBE1] dark:bg-[#0B1215]">
          <ul className="flex flex-col">
            {links.map((l) => {
              const isActive = active === l.id;
              return (
                <li key={l.label} className="border-b border-[#2A3530]/12 dark:border-[#E8E6DF]/12">
                  <a
                    href={l.href}
                    onClick={() => setOpen(false)}
                    className={`block py-4 text-[15px] transition-colors ${
                      isActive
                        ? "text-[#A07F2E] dark:text-[#C9A24B]"
                        : "text-[#2A3530] dark:text-[#E8E6DF] hover:text-[#A07F2E] dark:hover:text-[#C9A24B]"
                    }`}
                  >
                    {l.label}
                  </a>
