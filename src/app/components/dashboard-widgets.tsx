...
import { CloudRain, Thermometer, Droplets, TrendingUp, AlertTriangle, UploadCloud } from "lucide-react";
...
export function ClimateKpiCard() {
  const items = [
    { icon: <CloudRain size={16} strokeWidth={1.6} />, value: "150 mm", label: "Curah Hujan" },
    { icon: <Thermometer size={16} strokeWidth={1.6} />, value: "28°C", label: "Suhu" },
    { icon: <Droplets size={16} strokeWidth={1.6} />, value: "80%", label: "Kelembapan" },
  ];
    ...
    <Card title="Rata-Rata Iklim (NASA POWER)" eyebrow="Iklim">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {items.map((it) => (
          ...
          >
            <span className="inline-flex w-8 h-8 sm:w-7 sm:h-7 items-center justify-center rounded-md bg-[#C9A24B]/15 text-[#A07F2E] dark:text-[#C9A24B] shrink-0 sm:mb-2">
              {it.icon}
            </span>
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-[#5F6A64] dark:text-[#B8BFB9] mt-0.5">
                {it.label}
              </p>
            </div>
          </div>
    ...
    <Card title="Proyeksi Produksi (LSTM)" eyebrow="Prediksi">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[12px] text-[#5F6A64] dark:text-[#B8BFB9] mt-1">Ton</p>
        </div>
        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-[#7A9A6E]/15 text-[#4A7A40] dark:text-[#84B878] text-[11px]">
  ...
  return (
    <Card title="Status Wilayah" eyebrow="Risiko">
      <div className="flex items-center gap-4">
        <span className="inline-flex w-11 h-11 items-center justify-center rounded-xl bg-[#C9A24B]/15 text-[#A07F2E] dark:text-[#C9A24B] shrink-0">
...
export function PriorityTableCard() {
  ...
  const badge = (k: string) =>
    k === "Tinggi"
      ? "bg-[#C9A24B]/15 text-[#A07F2E] dark:text-[#C9A24B]"
        ...
        {rows.map((r) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className={`inline-flex whitespace-nowrap px-2 py-0.5 rounded-full text-[11px] ${badge(r.kategori)}`}>
                [ {r.kategori} ]
              </span>
            </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[11px] uppercase tracking-wider text-[#5F6A64] dark:text-[#A8AFA9]">
              ...
              <th className="px-2 py-2 font-normal text-right">Estimasi (Ton)</th>
            </tr>
          </thead>
          <tbody className="text-[13px] text-[#2A3530] dark:text-[#E8E6DF]">
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-2 py-3 text-right font-mono whitespace-nowrap">[ {r.estimasi} ]</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <Card title="Diagnosis Penyakit Cepat" eyebrow="Deteksi">
      <div className="rounded-xl border-2 border-dashed border-[#2A3530]/15 dark:border-[#E8E6DF]/15 bg-white/30 dark:bg-white/[0.04] px-4 py-8 flex flex-col items-center justify-center text-center">
        <span className="inline-flex w-10 h-10 items-center justify-center rounded-full bg-[#C9A24B]/15 text-[#A07F2E] dark:text-[#C9A24B] mb-3">
      ...
      <div className="mt-4 flex items-center justify-between text-[12px] text-[#5F6A64] dark:text-[#B8BFB9]">
        <span>
          Diagnosis: <span className="text-[#2A3530] dark:text-[#E8E6DF]">[ Menunggu Data ]</span>
        </span>
        <span>
          Akurasi: <span className="text-[#2A3530] dark:text-[#E8E6DF]">[ 0% ]</span>
        </span>
      </div>
    </Card>
  );
