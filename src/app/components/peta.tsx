...
const KalimantanMap = memo(function KalimantanMap() {
  ...
  return (
    <svg viewBox="0 0 600 420" className="w-full h-auto">
      <defs>
        <pattern id="petagrid" width="30" height="30" patternUnits="userSpaceOnUse">
          <path d="M 30 0 L 0 0 0 30" fill="none" stroke="currentColor" strokeOpacity="0.05" strokeWidth="0.5" />
        </pattern>
      </defs>

      <rect width="600" height="420" fill="url(#petagrid)" className="text-[#2A3530] dark:text-[#E8E6DF]" />
      ...
      <g className="text-[#5F6A64] dark:text-[#A8AFA9]" fill="currentColor" fontSize="9" fontFamily="monospace" letterSpacing="2">
        <text x="18" y="28">N</text>
        <line x1="20" y1="35" x2="20" y2="58" stroke="currentColor" strokeWidth="0.8" />
        <text x="18" y="402">S</text>
      </g>

      <g className="text-[#5F6A64] dark:text-[#A8AFA9]" stroke="currentColor" strokeWidth="0.8" fill="none">
      ...
      <text x="500" y="385" className="text-[#5F6A64] dark:text-[#A8AFA9]" fill="currentColor" fontSize="8" fontFamily="monospace" letterSpacing="1">0 — 200 KM</text>
    </svg>
  );
});
  ...
  return (
    <div className="group flex items-center justify-between py-3 px-2 -mx-2 rounded-md border-b border-[#2A3530]/15 dark:border-[#E8E6DF]/12 last:border-b-0 cursor-pointer transition-colors hover:bg-[#2A3530]/4 dark:hover:bg-[#E8E6DF]/4">
      <div className="flex items-center gap-3 min-w-0">
        <span className={`w-1.5 h-1.5 rounded-full shrink-0 transition-transform group-hover:scale-150 ${dotClass}`} />
        <div className="min-w-0">
          <div className="font-serif text-[15px] text-[#2A3530] dark:text-[#E8E6DF] truncate transition-colors group-hover:text-[#A07F2E] dark:group-hover:text-[#C9A24B]">
            {name}
          </div>
      ...
      <div className="font-mono text-[12px] text-[#4A5550] dark:text-[#B8BFB9] shrink-0 ml-3 transition-colors group-hover:text-[#A07F2E] dark:group-hover:text-[#C9A24B]">
        {value}
      </div>
    </div>
  );
      ...
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-10 mb-12 sm:mb-16">
        <div className="lg:col-span-5">
          <div className="flex items-center gap-3 mb-8 sm:mb-12">
            <span className="font-mono text-[11px] tracking-[0.2em] uppercase text-[#A07F2E] dark:text-[#C9A24B]">03</span>
          ...
          <Reveal>
            <h2 className="font-serif text-[32px] sm:text-[40px] lg:text-[52px] leading-[1.05] tracking-[-0.02em] text-[#2A3530] dark:text-[#E8E6DF]">
              Lanskap prioritas <em className="italic text-[#A07F2E] dark:text-[#C9A24B]">Kalimantan</em>, dalam satu pandangan.
            </h2>
          </Reveal>
        </div>
      ...
      <div className="grid lg:grid-cols-12 gap-px bg-[#2A3530]/12 dark:bg-[#E8E6DF]/12 rounded-2xl overflow-hidden">
        ...
        <aside className="lg:col-span-5 bg-[#F7F3EA] dark:bg-[#0F181B] p-6 sm:p-7 lg:p-9 flex flex-col gap-7">
              ...
              {[
                ...
                <div key={i.l} className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-[#5F6A64] dark:text-[#A8AFA9]">{i.l}</span>
                  </div>
          ...
          <div>
            ...
            <RegionRow name="Kutai Kartanegara" prov="Kaltim" value="5.8 t/ha" level="high" />
            <RegionRow name="Kotawaringin Timur" prov="Kalteng" value="5.4 t/ha" level="high" />
            <RegionRow name="Sintang" prov="Kalbar" value="4.6 t/ha" level="med" />
            <RegionRow name="Tanah Bumbu" prov="Kalsel" value="3.2 t/ha" level="low" />
          </div>
        </aside>
      </div>
    </section>
